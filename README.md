...
To calculate the binned tornado mask, first a mesh grid (`X_mask` and `Y_mask`) is created from `xedges_global` and `yedges_global`, on which the mask and the data will be visualised together. The mask itself is initialized as a numpy array of zeros whose size equals to that of the binned data histogram (which is a 2D numpy array as well). A bin cell value in the mask is set to 1 if it falls in between the 2 timewalk curves. In case a cell is on the boundary of the tornado mask, i.e. the timewalk curve crosses the cell, the fraction of the cell that is below the curve will be calculated. This whole process is done in a loop over all mask cells by using coordinate geometry in the [`src.getAreaBelowCurve()`](https://gitlab.cern.ch/pkicsiny/mib_rates/-/blob/master/src.py#L196) helper function. It is important that in the code 2 spearate masks are created this way, one to be applied on BIB and another to be applied on PU data. These 2 masks are subsequent in time by 25 ns difference i.e. if the 2 arrays are plotted together it looks like one mask having a width of 50 ns. The 2 masks have the same binning and it is also important to align the timewalk curves in the previous notebook cells such that they fall within the time axis range of the data. At this point the mask location on the time axis is not optimized and the purpose is only to have a 2D array displaying the tornado plot in full extent, with the same binning as the data.
